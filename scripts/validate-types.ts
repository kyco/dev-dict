...
const TYPES_DIR = join(__dirname, '../data/types')
...
function validateTypes(): void {
  const files = readdirSync(TYPES_DIR).filter((file) => file.endsWith('.json') && !SKIP_FILES.includes(file))

  const errors: ValidationError[] = []
  const seenIds = new Map<string, string>()
  const seenNames = new Map<string, { file: string; locale: string }>()

  console.log(`Validating ${files.length} type file(s)...\n`)

  // Load and validate all types
